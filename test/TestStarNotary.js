...
it("lets 2 users exchange stars", async () => {
  ...
  await instance.createStar("awesome star 7", 7, { from: user1 });
  await instance.createStar("awesome star 8", 8, { from: user2 });

  // 2. Call the exchangeStars functions implemented in the Smart Contract
  await instance.exchangeStars(7, 8, { from: user1 });
  //await instance.exchangeStars(8, 7, { from: user2 });
...
it("lets a user transfer a star", async () => {
  ...
  let user2 = accounts[2];
  // 1. create 2 Stars with different tokenId
  await instance.createStar("awesome star 9", 9, { from: user1 });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  let instance = await StarNotary.deployed();
  let user1 = accounts[1];
  await instance.createStar("awesome star 10", 10, { from: user1 });
  // 2. Call your method lookUptokenIdToStarInfo
  const name = await instance.lookUptokenIdToStarInfo(10);
  // 3. Verify if you Star name is the same
  assert.equal(name, "awesome star 10");
});
